...
ll_f = function(X, Y, beta, Sigma, rho) {
  p = dmvnorm(X, mean = rep(0, 7), sigma = diag(rep(Sigma, 7)), log = TRUE)
...
logpost_all = function(X, Y, beta, rho, Sigma){    
  return(c(ll_beta(beta) + ll_sigma(Sigma), ll_sigma(Sigma), ll_rho(rho)) + ll_f(X, Y, beta, Sigma, rho))
}
...
for (i in 2:nrep) {
  mchain[i, ] = MH_step(train_X = XX[10, ], # estimates must be made for each row (hour t)
                        train_Y = YY[10], 
                        params = mchain[i - 1, ], 
                        b_avec = c(0.5, 0.25, 0.25, 0.5, 0.5, 0.5, 0.5),
                        r_a = 0.05,
                        sig_a = 0.1)
}
...
for (k in 1:ncol(mchain)) {
  plot(mchain[1001:2000, k], type = "l", ylab = k - 1)
}

numunique(mchain)
for (i in 1:ncol(mchain)) {
  cat(i, "\t", mean(mchain[1001:2000,i]), "\n")
} 
```
