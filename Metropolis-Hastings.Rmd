...
Y = train_test %>% select(wind_kt) %>% as_vector()
```
...
logpost = function(X, Y, beta, rho, sigma){
   return(sum(loglik(X, Y, beta, rho, sigma) + log(dmvnorm(beta, rep(0,21), diag(1,21))) + log(dtruncnorm(rho, a=0, b=1, mean = 0.5, sd = 1/5)) + log(rinvgamma(sigma, 0.001, 0.001))))
}
```
...
comp_MCMC = function(train_X, train_Y, niter, beta, rho, sigma, beta_avec, rho_a, sigma_a){
  beta_matrix[1,] = beta
  rho_vec[1] = rho
  sigma_vec[1] = sigma
...
#start iterating 
MCMC_res = comp_MCMC(train_X = X, train_Y = Y, 1000, beta = rep(1,7), rho = 0.7, sigma = 2, beta_avec, rho_a, sigma_a)
```
